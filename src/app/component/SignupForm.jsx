...
//inital state to save user input
const initialState = {
  username: "",
  email: "",
  password: "",
...
const SignupForm = () => {
  ...
  // handlechange function to store user input value in initail state above
  const handleChange = (event) => {
    setError("");
    setState({ ...state, [event.target.name]: event.target.value });
  };
  // handle submit to handle Signup API
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // hitting signup API using axios
      const response = await axios.post("api/signup", newUser);
      console.log(response);

      //throwing message base on response is successfull or not
  ...
  // using this to prevent hydration error
  useEffect(() => {
    setHydrated(true);
  }, []);
